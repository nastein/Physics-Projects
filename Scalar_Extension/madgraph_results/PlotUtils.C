...
public:
    Plot(string id);
    void Add(string leg, int color, int style, TH1D* ph, bool normalize = false);
    void Add(string leg, int color, int style, TProfile* ph);
    void Add(string leg, int color, int style, int fillstyle, TH1D* ph, bool normalize = false);
    ...
    void DrawGraphRatio(string varX, string varY, const double posX, bool ratiolog = false, const double scale_axis = 1.1, const double min_y = 0);
...
void Plot::Add(string leg, int color, int style, TH1D* ph, bool normalize) {
    Add(leg, color, style, 0, ph, normalize);
}


void Plot::Add(string leg, int color, int style, int fillstyle, TH1D* ph, bool normalize) {
...
void Plot::Draw(string varX, string varY, const double posX, const double scale_axis, const double min_y) {
    ...
    for (unsigned int i = 0; i < num_items_; i++) {
        ...
        if (min_y !=0 ) h->SetMinimum(min_y);
        else if (log_) h->SetMinimum(0.001);
        else h->SetMinimum(0);
    ...
    for (unsigned int k =0; k <= 1; k++) {
        ...
        for (unsigned int i =0; i < num_items_; i++) {
            ...
            if (k == 0) {
                ...
                h_ratio[i]->SetMinimum(0.0001);
                h_ratio[i]->SetMaximum(10);
                h_ratio[i]->Draw("ep same");
                //Plot::Drawline(20, 1, 100, 1);
                h_ratio[i]->SetTitle("");
                h_ratio[i]->GetYaxis()->SetTitle("Ratio to SM 3#gamma bkgd");
...
void Plot::DrawGraphRatio(string varX, string varY, const double posX, bool ratiolog, const double scale_axis, const double min_y) {
    ...
    TGraph* h[num_items_];
    TGraph* h_ratio[num_items_];

    TGaxis::SetMaxDigits(3);

    double maxY = 0.0;
    double maxRatioY = 0.0;
    for (unsigned int i = 0; i < num_items_; i++) {
        TGraph* h = Analysis_graph_[i];
        int n = h->GetN();
        double* y = h->GetY();
    ...
    for (unsigned int k =0; k <= 1; k++) {
        ...
        for (unsigned int i =0; i < num_items_; i++) {
            ...
            if (k == 0) {
                h[i] = Analysis_graph_[i];

                if (i == 0) {
                    h[i]->GetYaxis()->SetRangeUser(0,maxY*1.2);  /// \todo : this is not well done, don't change h
                    if (log_) h[i]->SetMinimum(0.000001);
                    else if (min_y !=0 ) h[i]->SetMinimum(min_y);
                    else h[i]->SetMinimum(0.0);
                    h[i]->GetYaxis()->SetTitle("Efficiency of H_{125} reconstruction");
                ...
                if(i == 5) continue;
                h_ratio[i] = (TGraph*) h[i]->Clone();

                //h_ratio[i]->Sumw2();
                for(int b = 0; b < h_ratio[i]->GetN(); b++) {
                    h_ratio[i]->SetPoint(b,h_ratio[i]->GetPointX(b), h_ratio[i]->GetPointY(b)/h[0]->GetPointY(b));
                }

                h_ratio[i]->SetMinimum(0.05);
                h_ratio[i]->SetMaximum(100.0);
                h_ratio[i]->SetLineColor(h[i]->GetLineColor());
                if(i == 0)
                    h_ratio[i]->Draw();
                else
                    h_ratio[i]->Draw("same");
                ...
                //h_ratio[i]->GetYaxis()->SetLabelFont(43); // Absolute font size in pixel (precision 3)
                //h_ratio[i]->GetYaxis()->SetLabelSize(15);
                //h_ratio[i]->GetXaxis()->SetLabelSize(0.04);
                h_ratio[i]->GetYaxis()->SetLabelSize(0.06);
