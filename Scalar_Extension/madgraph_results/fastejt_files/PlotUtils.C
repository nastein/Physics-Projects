    ...
    for (unsigned int k =0; k <= 1; k++) {
        ...
        for (unsigned int i =0; i < num_items_; i++) {
            ...
            if (k == 0) {
                ...
                double average = 0;
                double average_err_sq = 0;

                for (int a = 1; a <= 50; a++) {
                    average += h_ratio[i]->GetBinContent(a);
                    average_err_sq += pow(h_ratio[i]->GetBinError(a),2);
                }

                std::cout << "Average = " << average/50 << std::endl;
                std::cout << "Error on the Average = " << pow(average_err_sq, .5)/50 << std::endl;
