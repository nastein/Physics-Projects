...
(******* NOTE: this package has been updated with suggestions from *******
 ******* Dan Roythorne.                                            *******)
...
ptwf[2][state_,ham0_,ham1_,ham2_] := Block[{e,h,h2,dim},
dim = Dimensions[ham1][[1]];
h2[i_,j_]:=ham2[[i,j]];
h[i_,j_] := ham1[[i,j]];
e[i_] := ham0[[i,i]];
Table[If[ state == j, Sum[If[l==state,0,(-1/2) (h[state,l] h[state,l])/((e[state]-e[l])^2)],{l,1,dim}],
(h2[state,j])/(e[state]-e[j]) +
Sum[If[k == state, 0, (h[j,k]*h[k,state])/((e[state]-e[k])(e[state]-e[j])) ],{k,1,dim}] -
